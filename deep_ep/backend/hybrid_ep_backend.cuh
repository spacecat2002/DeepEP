...
public:
  ...
  static void combine(combine_kernel_param_t param, cudaStream_t stream)
  {
    ...
    // In the single-node non-fused path this kernel has no state to update:
    // the RDMA counter is disabled and no unpermute flag is compiled in.
#if defined(HYBRID_EP_BUILD_PERMUTE_FUSION_ENABLE)
